...
public class IOCommandsProcessor extends CommandProcessor {
	...
	/**
	 * Default constructor for IOCommandsProcessor objects
	 */
	...
	/**
	 * Prompts and reads the next user's command. If needed, the reading is
	 * repeated until the input has some content.
	 * 
	...
	/**
	 * Initializes the list whose output is controlled by this OutputProcessor
	 * object.
	 * 
	 * @param rList
	 *            the list
	...
	/**
	 * Sets the length of the block (number of lines) that will be shown
	 * whenever the output is being produced whenever the methods
	 * showNextBlock() or showPrevBlock() are applied to the current
	 * OutputProcessor object.
	 * 
	 * @param bl
	 *            the new length of the output block...
	 */
	...
	/**
	 * Outputs the next block, beginning from the line at currentIndex in the
	 * list.
	 */
	private void showNextBlock() {
		boolean moreLinesToShow = true;
		int lastLineToShow = Math.min(resultsList.size() - 1, currentIndex + outputBlockLength - 1);
		int firstLineToShow = currentIndex;
		currentIndex = lastLineToShow + 1;
		for (int line = firstLineToShow, count = 1; count <= outputBlockLength && moreLinesToShow; line++, count++) {
			try {
	...
	/**
	 * Outputs the previous block, where the last line will be the line at
	 * currentIndex-1 in the list.
	 */
	...
	/*
	 * private boolean hasMoreResultsPrev() { return currentIndex > 0; }
	 * 
	 * private boolean hasMoreResultsNext() { return currentIndex <
	 * resultsList.size(); }
	 */
	 ...
	 * "DeclareProcessor" class must be a "declare" command.
	 *
	 */

	/**
	 * Next processor inner class
	 * 
	 * @author pirvos
	 *
	 */
	...
	/**
	 * Set Rows Processor inner class
	 * 
	 * @author pirvos
	 *
	 */
	...
	/**
	 * Determines if there is more output to process
	 * 
