...
/**
 * This defines a command validator type. These type of validators are used for
 * those commands with a complex grammar in which the number of tokens in the
 * operand value is variable. For example: a list expression, etc. For such type
 * of command it holds the appropriate operand analyzers as the members of an
 * internal list. These are used to properly analyze the operands that the user
 * includes in a particular command instance of a system command to which the
 * CommandValidator is assigned to.
 * 
...
public class VariableLengthCommandValidator implements CommandValidator {
	...
	// this is the list of list of items extracted from all the operands (one
	// list per
	// operand) when the last input line from user is analyzed....

	/**
	 * Default constructor for VariableLenghtCommandValidator objects
	 */
	...
	/**
	 * Adds analyzers for operands on system
	 * 
	 * @param ops
	 *            the list of tokens in the grammar of the command.
	 */
	...
	/**
	 * Validates system command from input
	 * 
	...
	public boolean validate(SystemCommand corrSC, CommandLine userInput, ErrMsg errMsg) {
		...
		for (int i = 1; i <= nOperands; i++) {
			VariableLengthCommandValidator scv = (VariableLengthCommandValidator) corrSC.getCommandValidator();
	...
	/**
	 * Returns operand analyzer for index value
	 * 
	 * @param index
	 *            index of operand
	 * @return analyzer for operand
	 */
